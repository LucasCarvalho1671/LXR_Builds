...
aiForm.addEventListener("submit", async (event) => {
  ...
  if (selectedGame === "lol" && wantsSummonerInfo) {
    ...
    if (summonerName === "" || summonerTag === "" || platformRegion === "") {
        alert("Por favor, preencha todos os campos do invocador (Nome, Tag e Região).");
  ...
  try {
    ...
    aiResponse.querySelector(".response-content").innerHTML = markdownToHTML(data.response);
    showElement(aiResponse);
  } catch (error) {
    console.error("Erro ao obter resposta da IA:", error);
    aiResponse.querySelector(".response-content").innerHTML =
      `<p style="color: red;">Ocorreu um erro: ${error.message}. Tente novamente mais tarde.</p>`;
    showElement(aiResponse);
  } finally {
    askButton.disabled = false;
    askButton.textContent = "Perguntar";
